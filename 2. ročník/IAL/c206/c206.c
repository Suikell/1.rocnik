...
/**
 * Abstrakce nad metodami pro mazani prvku seznamu.
 * Smaze prvek 'element' v parametru a v pripade,
 * ze byl pred nebo za prvkem jiny prvek nastavi jeho odkaz na dalsi/predchozi prvek prislusnym zpusobem.
 * Osetruje situace pro zacatek a konec prvku.
 * @param list Ukazatel na strukturu dvousměrně vázaného seznamu
 * @param element Prvek, ktery chceme smazat
 */
void DLL_Delete(DLList *list, struct DLLElement *element) {
    if (element == NULL) return;
    if (list->activeElement == element) list->activeElement = NULL;
...
void DLL_InsertFirst(DLList *list, int data) {
    struct DLLElement *new = calloc(1, sizeof(struct DLLElement));
    if (!new) {
        DLL_Error();
        return;
    }
...
void DLL_DeleteFirst(DLList *list) {
    DLL_Delete(list, list->firstElement);
}
...
void DLL_DeleteLast(DLList *list) {
    ...
    DLL_Delete(list, list->lastElement);
}
...
void DLL_DeleteAfter(DLList *list) {
    if (list->activeElement) {
        DLL_Delete(list, list->activeElement->nextElement);
    }
}
...
void DLL_DeleteBefore(DLList *list) {
    if (list->activeElement) {
        DLL_Delete(list, list->activeElement->previousElement);
    }
}
...
void DLL_SetValue(DLList *list, int data) {
    ...
    if (DLL_IsActive(list)) list->activeElement->data = data;
}
